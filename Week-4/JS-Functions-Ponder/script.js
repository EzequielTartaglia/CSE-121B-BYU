function getGrades(inputSelector) {
    // get grades from the input box (Get the input #grade)
    const grades = document.querySelector(inputSelector).value;
    // split them into an array (String.split(','))
    const gradesArray = grades.split(",");
    // clean up any extra spaces (.trim() do that), and make the grades all uppercase. (Array.map())
  ...
  function calculateGpa(grades) {
    ...
    // calculates the GPA (averange)
    const gpa =
      gradePoints.reduce((total, num) => total + num) / gradePoints.length;
    // return the GPA (2 places after the coma)
